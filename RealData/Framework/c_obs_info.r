...
for (k in 1:length(adjust_val)) {
    ...
    for (i in indexList_MAIN) {
        ...
        n_arr_1_prec = length(prec_1_x) 
        n_arr_2_prec = length(prec_2_x) 
        n_off_1_prec = length(prec_1_x_off)
        n_off_2_prec = length(prec_2_x_off) 
    ...
    save(origData, file=paste0("../Output/origGridInfo/origData_", k,".dat"))
}
