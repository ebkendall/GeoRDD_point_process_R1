...
for (index in 1:8) {
    ...
    for(k in 1:77) {
            ...
            for(j in 1:length(streetLengthInfo_null[[i]])) {
                if(sum(is.na(streetLengthInfo_null[[i]][[j]])) == 0) {
                    if(length(streetLengthInfo_null[[i]][[j]]$buffer@polygons) > 1){
                        ...
                        # Naive p-value
                        n_arr_1 = length(prec_1_x)     
                        n_arr_2 = length(prec_2_x)     
                        n_off_1 = length(prec_1_x_off) 
                        n_off_2 = length(prec_2_x_off) 
        ...
        save(nullData, file=paste0("../Output/nullGridInfo/nullData", 
            index, "_", k,".dat", sep=''))
