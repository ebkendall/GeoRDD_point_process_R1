...
save(indiv_results_theta, file = '../Output_tree/indiv_results_theta.rda')
save(indiv_results_tau, file = '../Output_tree/indiv_results_tau.rda')
...
load('../Output_tree/indiv_results_theta.rda')
load('../Output_tree/indiv_results_tau.rda')
...
for(i in 1:length(p_tau)) {
    ...
    p_tau[[i]] = ggplot(p_val_tau, aes( y=y, x=x)) +
        ...
        labs(title=TeX(r'($\tau$: Effect of $c$ on type I error)'), 
             subtitle=substitute(paste("Spatial smoothing multiplier (", 
             sigma," x ",m, ")"),list(m=adjust_val[i]))) +
        xlab(TeX(r'($c$)')) +
        ylab("Type I error") +
...
pdf("../Plots/match_theta.pdf", onefile = T)
...
pdf("../Plots/match_tau.pdf", onefile = T)
...
save(match_count_list, file = '../Output_tree/match_count_list.dat')
